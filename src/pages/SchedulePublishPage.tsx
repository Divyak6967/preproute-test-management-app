...
export function SchedulePublishPage() {
  ...
  useEffect(() => {
    ...
    async function loadTest() {
      if (!testId) {
        const message = 'Select a test before scheduling.'
        setError(message)
        toast.error(message)
  ...
  async function handleConfirm() {
    ...
    if (!questions.length) {
      const message = 'Add at least one question before scheduling.'
      setError(message)
      toast.error(message)
      return
    }

    if (!controls.scheduleDate || !controls.scheduleTime) {
      const message = 'Select schedule date and time.'
      setError(message)
      toast.error(message)
      return
    }
    ...
    try {
      ...
      await updateTest(testId, {
        status: 'scheduled',
        publish_date: controls.scheduleDate,
        publish_time: controls.scheduleTime,
      ...
      const message = getErrorMessage(scheduleError, 'Unable to schedule test.')
      setError(message)
      toast.error(message)
    } finally {
  ...
  return (
    <QuestionShell questions={questions} totalQuestions={test?.total_questions ?? questions.length}>
      ...
      <PublishControls
        ...
        scheduleDate={controls.scheduleDate}
        scheduleTime={controls.scheduleTime}
